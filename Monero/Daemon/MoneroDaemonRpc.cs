...
public class MoneroDaemonRpc : IMoneroDaemon
{
    ...
    public async Task<string> GetBlockHash(ulong height)
    {
        ...
        var respMap = await _rpc.SendJsonRequest("on_get_block_hash", param);
    ...
    {
        var respMap = await _rpc.SendJsonRequest<GetLastBlockHeaderResult>("get_last_block_header", new MoneroJsonRpcParams());
    ...
    public async Task SubmitBlocks(List<string> blockBlobs)
    {
        ...
        var resp = await _rpc.SendJsonRequest<MoneroRpcResponse>("submit_block", blockBlobs);
