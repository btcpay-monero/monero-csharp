...
public class MoneroRpcConnection(Uri uri, string username, string password, HttpClient? client = null)
{
    ...
    public async Task<MoneroJsonRpcResponse<T>> SendJsonRequest<T>(string method, List<string> parameters)
    {
    ...
    {
        return await SendJsonRequest<string>(new MoneroJsonRpcRequest(method, parameters));
    }
    ...
    private async Task<MoneroJsonRpcResponse<T>> SendJsonRequest<T>(MoneroJsonRpcRequest rpcRequest)
    {
    ...
    {
        return await SendPathRequest<T>(path, []);
    }
